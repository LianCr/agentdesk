...
**M7 后确立的界面规则:一屏一个现实动作**

- 每个页面的结尾只出现**一个**现实动作(去比较、去要资料、送交审核、下一条待审),
  由确定性代码从已核验状态选出;模型文本只作说明,不得决定链接目标或动作
- 指标、理由明细、技术状态退到折叠层或动作卡内部,给需要核查的人,不给需要行动的人
- 衡量标准:真实用户看完输出后要做的决定变少,而不是变多
