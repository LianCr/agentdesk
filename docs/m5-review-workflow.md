...
后端仍然不存勾选状态。原来的结论是「因此界面上也不放勾选框」——一个刷新就忘记的
勾选框看起来像记录,却不是记录。

清单现在可以逐条展开(为什么出现 / 需要核实 / 相关比较维度与出处),也可以逐条勾选,
但那个反对意见针对的是**假装成记录的持久化**,不是交互本身,所以边界划在这里:

- 勾选只活在当前标签页的 React state 里,不写数据库、不写 localStorage、
  不随决定提交(UI 测试 49 直接断言提交体恰好是 `[{ type: "approve" }]`)
- 刷新即清空,界面上明写「勾选仅用于本次查看,刷新后清空,不构成审核记录」
- 全部勾完只显示完成提示,**不改变批准 / 拒绝控件的任何行为**;提示本身也写明
  它不表示适合性、承保方核准或合规批准
